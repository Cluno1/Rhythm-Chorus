...
object VersionComparator {
    ...
    fun parse(versionString: String?, isPreRelease: Boolean? = null): SemanticVersion {
        ...
        try {
            ...
            // Tags are canonical, but accepting a human title such as "Sonorus 1.2.3"
            // keeps parsing defensive when an external caller passes presentation text.
            val numericBase = Regex("(?i)(?:^|[^0-9])v?(\\d+)\\.(\\d+)(?:\\.(\\d+))?(?:\\.(\\d+))?")
                .find(cleaned)
            val major = numericBase?.groupValues?.getOrNull(1)?.toIntOrNull() ?: 0
            val minor = numericBase?.groupValues?.getOrNull(2)?.toIntOrNull() ?: 0
            val patch = numericBase?.groupValues?.getOrNull(3)?.toIntOrNull() ?: 0
            val subpatch = numericBase?.groupValues?.getOrNull(4)?.toIntOrNull() ?: 0
