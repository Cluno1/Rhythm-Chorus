...
`.github/workflows/release.yml` performs the same fail-closed sequence for Stable after every push to `main`. It uses the separately protected `sonorus-stable` GitHub environment, the permanent Release APK certificate, and the Stable Ed25519 manifest key. Stable versions are allocated from that workflow's monotonically increasing run number: run `N` produces version name `1.(N / 1000).(N % 1000)` and version code `1,000,000 + N`. A failed test, build, signature check, Artifact transfer, COS full read-back, or server verification leaves the previous Stable `latest.json` unchanged.
